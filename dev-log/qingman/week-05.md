...
- **Added OTel setup as an app-level concern.** Created `app/observability.py`
  with idempotent OpenTelemetry setup: service name, environment resource
  attributes, FastAPI instrumentation, SQLAlchemy/psycopg instrumentation, and
...
- **Kept local development safe.** OTel is enabled by default, but spans are not
  ...
  tests and development do not require a collector, Honeycomb key, or AWS
  observability setup.
